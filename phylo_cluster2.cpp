...
void printLeaves(node* realroot, node *root) {
  ...
  if (root->isleaf)
    cout << root->info << '\t' << ancestor_distance(realroot, root) << '\n';
}
...
void flipBranches(node *root) {
  if (root->isleaf) return;
  if (root->ltree) flipBranches(root->ltree);
  if (root->rtree) flipBranches(root->rtree);
  ...
  */
  ...
  for (int i=1;i<=N;i++) {
    if (glp_mip_col_val(mip, i) == 1 ) {
      ...
      printLeaves(root, sel_nodes[i-1]);
      cout << endl;
    }
  }
  glp_delete_prob(mip);
